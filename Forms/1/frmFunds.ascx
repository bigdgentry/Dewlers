...
<xmod:FormView runat="server"><AddItemTemplate><xmod:AddForm runat="server">
  ...
  <SelectCommand CommandText="SELECT DisplayName as Username, ISNULL(ud.FundsAvailable, 0) as FundsAvailable FROM Users u LEFT JOIN tblUserData ud ON u.UserID = ud.UserID WHERE u.UserID = @UserID">
    <Parameter name="UserID" Value='<%#UserData("ID")%>'/>
  </SelectCommand>

  <SubmitCommand CommandText="spAlterFunds" CommandType="StoredProcedure">
    <Parameter name="Amount" />
    <Parameter name="Comment" Value="Added funds from Form" />
    <Parameter name="UserID" Value='<%#UserData("ID")%>'/>
  </SubmitCommand>

  <div class="xmp-form xmp-frmDewl1">
    <div class="xmp-form-row">
      Account for <b><Text DataField="Username" /></b><br/>
      Amount Available: <b><Text DataField="FundsAvailable" /></b>
    </div>
    <div class="xmp-form-row">
      <Label CssClass="xmp-form-label" For="Amount">Amount</Label>
      <TextBox Id="Amount" Width="250" Nullable="true" MaxLength="200" DataField="Amount" DataType="decimal" CssClass="txtAmt"></TextBox>
      <Validate Target="Amount" CssClass="xmp-validation" Type="compare" CompareValue="0" Operator="NotEqual" Text="***" Message="You must enter an amount"></Validate>
    </div>
    <ValidationSummary CssClass="xmp-validation" Id="vstblDewl"></ValidationSummary>
    <div class="xmp-form-row">
      <Label class="xmp-form-label">&nbsp;</Label>
      <AddButton Text="Add Dawllars"></AddButton>
      <CancelButton Text="Cancel" style="margin-left: 12px;" Visible="true"></CancelButton>
    </div>
  </div>
  </xmod:AddForm>
  </AddItemTemplate>
